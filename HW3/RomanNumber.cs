...
namespace HW3
{
    ...
    public static RomanNumber operator -(RomanNumber? n1, RomanNumber? n2)
    {
        if(n1 is null || n2 is null || ((n1.number <= n2.number))) throw new RomanNumberException();
        return new RomanNumber((ushort)(n1.number - n2.number));
    }
    public static RomanNumber operator *(RomanNumber? n1, RomanNumber? n2)
    {
    ...
    public static RomanNumber operator /(RomanNumber? n1, RomanNumber? n2)
    {
        if(n1 is null || n2 is null || (n1.number % n2.number != 0 ||
            n1.number < n2.number)) throw new RomanNumberException();
        return new RomanNumber((ushort) (n1.number / n2.number));
    }
